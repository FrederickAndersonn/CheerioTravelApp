...
function useDestination() {
  ...
  const handleDestinationSubmit = async () => {
    try {
      const trimmedDestinationData = {
        name: destinationData.name.trim(),
        description: destinationData.description.trim(),
        activities: destinationData.activities.filter(
          (activity) => activity.trim() !== ""
        ),
        photos: destinationData.photos.filter((photo) => photo.trim() !== ""),
      };

      await createDestination(trimmedDestinationData);
      ...
      alert(
        "Error creating destination." + JSON.stringify(error.response.data)
      );
      console.error(error);
    }
  };
  ...
  const handleDeleteDestination = async (tripId: string) => {
    try {
      ...
      alert(
        "Error deleting destination." + JSON.stringify(error.response.data)
      );
      console.error(error);
    }
  };
  ...
  const handleUpdateDestination = async () => {
    try {
      ...
      updatedData.activities = destinationData.activities.filter(
        (activity) => activity.trim() !== ""
      );
      updatedData.photos = destinationData.photos.filter(
        (photo) => photo.trim() !== ""
      );
      ...
      alert(
        "Error updating destination." + JSON.stringify(error.response.data)
      );
      console.error(error);
    }
  };
