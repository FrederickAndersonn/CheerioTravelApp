    ...
    const fetchData = async () => {
      try {
        ...
        const destinationDetails = await Promise.all(
          destinationIds.map(async (id: string) => {
            const exists = await checkDestinationExistence(id);
            if (exists) {
        ...
        setLoadedDestinations(filteredDestinations);
        setLoading(false);
      } catch (error) {
  ...
  const handleJoinNow = async () => {
    ...
    if (participantName) {
      try {
        ...
        alert(
          `Welcome aboard ${participantName}. You have been added to the trip.`
        );
      } catch (error) {
          ...
          <div className="w-1/2 h-full pr-4">
            ...
            <div className="flex justify-between bg-custom-black items-center mt-4 ">
              {timeLeft.split(" ").map((unit, index) => (
                <div
                  key={index}
                  className="flex flex-col justify-center items-center w-24 h-24 text-white rounded"
                >
            ...
            <div className="conatiner mx-auto flex items-center justify-center mb-16">
              {weather.map((weather: any, index: any) => (
                <div
                  key={index}
                  className="flex flex-col justify-center items-center w-[200px] h-[200px] bg-custom-black text-white rounded mt-16"
                >
          ...
          <div className="w-1/2 max-h-[calc(100vh-2rem)] h-full pl-4 overflow-y-auto">
            ...
            {loadedDestinations.map((destination: any, index) => (
              <div
                ...
                className="border border-slate-400 rounded-lg p-4  mb-2 shadow-md"
              >
                    ...
                    destination.photos.map(
                      (photo: string, photoIndex: number) => (
                        <img
                          ...
                          className="w-1/2 h-32 object-fit m-1"
                        />
                      )
                    )
                  ) : (
